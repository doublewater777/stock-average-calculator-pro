...
export default function StockCalculator() {
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem(
        "stock-calc-data",
        JSON.stringify({ rows, commissionType, commissionValue })
      );
    }
  ...
  const addRow = () => {
    setRows([
      ...rows,
      { id: Math.random().toString(36).substr(2, 9), price: "", quantity: "" },
    ]);
  };
    ...
    >
      ...
      <header className="mb-6 md:mb-10 flex flex-col items-center md:items-start md:flex-row md:justify-between md:gap-8">
        <div className="flex-1 text-center md:text-left pr-10 md:pr-0">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-2 md:mb-3">
            ...
            <span className="text-[8px] md:text-[9px] uppercase font-bold tracking-widest text-emerald-500">
              {t("calc.badge")}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-4 md:gap-8 items-start">
        ...
        <div className="space-y-4 md:space-y-6 order-2 lg:order-1">
          {/* Input Rows */}
          <fieldset className="space-y-3 md:space-y-4 border-0 p-0 m-0">
            <div className="flex items-center justify-between px-1">
              <legend className="text-[10px] md:text-xs font-bold text-[var(--text-dim)] uppercase tracking-widest flex items-center gap-2">
                <Plus size={12} className="text-blue-500" aria-hidden="true" />
              ...
              >
                <RotateCcw size={10} aria-hidden="true" /> {t("calc.reset")}
              </button>
            </div>
                    ...
                    <div className="space-y-1 md:space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          aria-label={`Purchase price for entry ${
            ...
            <button
              ...
              aria-label="Add another purchase entry"
            >
              <div
                className="p-0.5 md:p-1 rounded-md bg-[var(--input-bg)] group-hover:bg-[var(--accent-primary)] group-hover:text-black transition-all"
                aria-hidden="true"
              >
                <Plus size={12} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Commission Logic */}
            <fieldset className="glass-card p-4 md:p-5 bg-[var(--surface-hover)]/30 border-0">
              <div className="flex items-center justify-between mb-3 md:mb-4">
                <legend className="text-[9px] md:text-[10px] font-bold text-[var(--text-dim)] uppercase tracking-widest flex items-center gap-1.5 md:gap-2">
                  <Percent
                    size={10}
                ...
                <div
                  className="flex bg-[var(--input-bg)] p-0.5 rounded-lg border border-[var(--border)]"
                  role="radiogroup"
                >
                  {["percent", "flat"].map((type) => (
                    <button
                      ...
                      role="radio"
                      aria-checked={commissionType === type}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  aria-label={`Commission ${
                    commissionType === "percent" ? "percentage" : "flat fee"
                  }`}
                />
              </div>
            </fieldset>
            ...
            <fieldset className="glass-card p-4 md:p-5 border-blue-500/20 bg-blue-500/5">
              <legend className="text-[9px] md:text-[10px] font-bold text-blue-500 dark:text-blue-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                <CalcIcon size={12} aria-hidden="true" />
                {t("target.title")}
              </legend>
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-[8px] md:text-[9px] font-bold text-[var(--text-dim)] uppercase">
                    {t("target.price")}
                  </label>
                  <input
                    type="number"
                    ...
                    aria-label="Target average price"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[8px] md:text-[9px] font-bold text-[var(--text-dim)] uppercase">
              ...
              {results.requiredShares > 0 && (
                <div
                  className="mt-2.5 pt-2.5 border-t border-blue-500/10 flex justify-between items-center text-[10px]"
                  role="status"
                  aria-live="polite"
                >
                  <div>
                    <span className="text-[var(--text-dim)]">
                      {t("target.result_shares")}:{" "}
                    </span>
                    <span className="font-bold text-blue-500">
                      {Math.ceil(results.requiredShares).toLocaleString()}
                    </span>
                  </div>
                  <div>
                    <span className="text-[var(--text-dim)]">
                      {t("target.result_cost")}:{" "}
                    </span>
                    <span className="font-bold text-blue-500">
                      $
                      {results.requiredCost.toLocaleString(undefined, {
          ...
          >
            ...
            <div className="p-4 md:p-6">
              ...
              <div className="flex flex-col gap-0.5 md:gap-1 mb-4 md:mb-8 overflow-hidden">
                <div className="text-[10px] md:text-xs text-[var(--text-dim)] font-medium">
                  {t("results.avg_price")}
                </div>
                <div className="text-2xl md:text-4xl font-black text-[var(--foreground)] font-mono flex items-baseline gap-1.5 break-all leading-tight">
                  <span className="text-lg md:text-xl text-[var(--accent-primary)]">
                    $
                  </span>
                  {averagePriceStr}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4 pt-4 md:pt-6 border-t border-[var(--border)]">
                <div className="space-y-0.5 md:space-y-1">
                  <div className="text-[8px] md:text-[9px] text-[var(--text-dim)] uppercase font-bold tracking-wider">
                    {t("results.total_shares")}
                  </div>
            ...
            <div className="bg-emerald-500/5 p-3 md:p-4 border-t border-[var(--border)] flex items-start gap-2">
              ...
              <p className="text-[9px] md:text-[10px] text-[var(--text-dim)] italic leading-relaxed">
                {t("results.holding_statement", {
                  price: results.averagePrice.toFixed(2),
                })}
              </p>
            </div>
          </div>
